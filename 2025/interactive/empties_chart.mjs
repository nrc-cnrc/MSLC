...
const DOMAIN = ["punct", "word", "phrase", "segment",];
...
function create_graph(datum) {
  ...
  const graph = Plot.plot({
    ...
    marginBottom: 55,
    marginLeft: 70,
    // WARN: We must set the range or otherwise dot with highest frequency are not showing.
    r: { range: [0, 6] },
    x: {
      label: "score",
      tickRotate: 90,
...
async function get_data(data_file, metricminmax_file) {
  ...
  const common_metrics = Array.from(new Set(Object.keys(metrics))
    .intersection(new Set(Object.keys(metricminmax))));
